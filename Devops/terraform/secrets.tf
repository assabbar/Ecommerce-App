...
resource "kubernetes_secret" "cosmosdb_credentials" {
  ...
  data = {
    "uri" = "mongodb://${azurerm_cosmosdb_account.mongo.name}:${azurerm_cosmosdb_account.mongo.primary_readonly_master_key}@${azurerm_cosmosdb_account.mongo.name}.mongo.cosmos.azure.com:10255/?ssl=true&replicaSet=globaldb&retryWrites=false&maxIdleTimeMS=120000&appName=@${azurerm_cosmosdb_account.mongo.name}@"
  }
